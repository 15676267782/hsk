...
【题目规则】
1.如果不确定与用户请求相关的文件内容或代码库结构，请使用您的工具读取文件并收集相关信息，不要猜测或编造答案
2.在每次函数调用之前，必须进行广泛的计划，并对之前函数调用的结果进行广泛的反思。不要只通过调用函数来完成整个过程，因为这会削弱你解决问题和深入思考的能力
3.共生成{num_questions}道题。
4.保持题型多样性。
5.难度符合HSK{level}大纲。
6.每次启动程序生成的题目都要不一样，要十分严格的执行这一条规则。
7.要在原有的选项中设置干扰项，干扰强度随着HSK等级逐级提升，不要为了干扰项增加选项的数量。
8.生成的图片尽量写实，最好是真人的。
9.图片排序图是五段毫不相关的dialogues。
10.所有的题目都只能是单选题。
11.生成的图片与文字描述要一致。
12.生成{QUESTION_TYPES}带有‘判断题’的题的时候必须要保持正确答案和错误答案各站百分之五十的概率
...
def get_examples():
    ...
    return [
        ...
        {
            "type": "文字判断题",
            "audio_content": "很多人年轻时觉得钱比健康重要，老了以后才发现自己错了，很多东西都是这样，等到没有了我们才明白他的重要性",
            "target_sentence": "我们要重视身体健康",
            "answer": "对"
        },
        {
            "type": "问答匹配题",
        ...
        {
            "type": "阅读理解题",
            "passages": ["网球袜和普通的袜子有很大的区别。网球袜一般都很厚，因此它能很好地吸汗。在紧张的网球运动过程中，脚会受很大的压力，如果选择厚厚的网球袜，就能减少脚底的压力，更好地保护你的脚。"],
            "question": ["这段话主要讲了选择厚网球袜的什么？"],
            "options": ["条件", "原因", "方法","要点"],
            "answer": "B"
        },
        {
        ...
        },
        {
            "type": "短文选词填空题6",
            "passages": [
                "现在人们工作生活节奏都很快，办事都（1）效率，对养生也常常（2），希望一个养生法子几天就能见效。才过十天半月，就想把几年甚至几十年积累的问题（3）；又缺乏耐心，好了伤疤忘了痛，身体（4）有了起色就又开始纵容自己，把养生的成果在（5）期就给毁掉了，实在是非常可惜。"
            ],
